...
export default function CoursesPage() {
  ...
  const fetchCourseData = async () => {
    ...
    // Usar dados mock
    const stagesWithCourses = mockStages.map(stage => ({
      id: stage.id,
      title: stage.name,
      subtitle: '',
      courses: mockCourses
        .filter(course => course.stage_id === stage.id)
        .map(course => ({
          id: course.id,
          title: course.title,
          description: '',
    ...
    setStages(stagesWithCourses);

    // Simular cursos completados
  ...
  const handleCompleteCourse = async (courseId: number) => {
    ...
    console.log(`Curso ${courseId} marcado como completado`);
  };
