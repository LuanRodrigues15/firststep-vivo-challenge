...
import { mockAccess, mockOnboardees } from "@/data/mockData";
...
export function RHAccessesTab({ onChange }: { onChange?: () => void }) {
  ...
  const loadAccesses = () => {
    setAccesses(mockAccess);
  };

  const loadOnboardees = () => {
    const onboardeeList: Onboardee[] = mockOnboardees.map(onboardee => ({
      id: onboardee.id,
      name: onboardee.name
    }));
    setOnboardees(onboardeeList);
  };

  const loadPermissions = (onboardeeId: string) => {
    // Simular permissões localmente
    setPermissions([]);
  };
