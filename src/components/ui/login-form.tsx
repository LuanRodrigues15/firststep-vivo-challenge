...
import { Mail, Lock, Eye, EyeOff, Sun, Moon, LogIn } from "lucide-react";
...
const QUICK_LOGINS = [
  { role: 'RH', email: 'rh@example.com', password: 'rh123456', color: 'bg-blue-600 hover:bg-blue-700' },
  { role: 'Buddy', email: 'buddy@example.com', password: 'buddy123456', color: 'bg-green-600 hover:bg-green-700' },
  { role: 'Onboardee', email: 'onboardee@example.com', password: 'onboardee123456', color: 'bg-purple-600 hover:bg-purple-700' },
];
...
export function LoginForm({
  ...
  const handleQuickLogin = (quickLogin: typeof QUICK_LOGINS[0]) => {
        ...
        <div className="space-y-6">
          ...
          {!showForgotPassword ? <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-3 pt-4 border-t border-border">
                <p className="text-xs text-muted-foreground text-center font-medium">OU USE ACESSO RÁPIDO</p>
                <div className="grid grid-cols-3 gap-3">
                  {QUICK_LOGINS.map((quickLogin) => (
                    <Button
                      key={quickLogin.role}
                      type="button"
                      onClick={() => handleQuickLogin(quickLogin)}
                      className={`h-10 text-sm font-medium text-white ${quickLogin.color} transition-colors`}
                    >
