...
import { mockAccess } from "@/data/mockData";
...
export function AccessManagement({ user }: AccessManagementProps) {
  ...
  useEffect(() => {
    const loadAccesses = async () => {
      ...
      // Usar dados mock
      const formattedAccesses: SystemAccess[] = mockAccess
        .map((access: any) => ({
          id: access.id.toString(),
          systemName: access.name,
          description: '',
          logo: undefined,
          status: access.status as 'aprovado' | 'pendente' | 'recusado',
        }))
        .sort((a, b) => {
